...
function processFile(file) {
  ...
  const before1 = src;
  src = src.replace(/^\s*;\s*\(\s*['"]use client['"]\s*\)\s*;?\s*$(\r?\n)?/gm, '');
  ...
  if (/export\s+const\s+metadata\s*=/.test(fixed)) {
    issues.push({ file, issue: 'Composant client exporte `metadata` (à déplacer/supprimer).' });
  }
}
...
if (issues.length) {
  ...
  for (const it of issues) console.log('   -', path.relative(process.cwd(), it.file), '→', it.issue);
}
