...
export async function GET() {
  ...
  try {
    ...
    if (!customerId) return NextResponse.json({ hasActive: false, entitlements: [] });
    ...
    const entitlements = subs.data.map((s) => {
      const m = s.metadata || {};
      return {
        id: s.id,
        status: s.status,
    ...
    return NextResponse.json({ hasActive: entitlements.length > 0, entitlements });
  } catch (e) {
